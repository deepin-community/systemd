...
static int device_get_sysattr_u64_compat(sd_device *device, const char *sysattr, uint64_t *ret) {
        const char *value;
        int r;
...
static int strdup_to_compat(char **ret, const char *value) {
        char *copy;
        ...
        free_and_replace(*ret, copy);
        return 0;
}

static int device_sysname_startswith_compat(sd_device *device, const char *prefix) {
        const char *sysname;
        int r;
        ...
        return startswith(sysname, prefix) != NULL;
}

static int blockdev_partscan_enabled_compat(sd_device *device) {
        _cleanup_close_ int fd = -EBADF;

        assert(device);

        fd = sd_device_open(device, O_RDONLY|O_CLOEXEC|O_NONBLOCK|O_NOCTTY);
        if (fd < 0)
                return fd;

        return blockdev_partscan_enabled(fd);
}
...
static int blockdev_get_prop(sd_device *d, const char *prop1, const char *prop2, char **ret_value) {
        ...
        FOREACH_STRING(prop, prop1, prop2) {
                ...
                if (r < 0 && r != -ENOENT)
                        ...
                        return strdup_to_compat(ret_value, m);
        }
...
static int blockdev_get_subsystem(sd_device *d, char **ret_subsystem) {
        ...
        for (;;) {
                ...
                if (r < 0)
                        ...
                        return strdup_to_compat(ret_subsystem, s);
        }
...
int blockdev_list(BlockDevListFlags flags, BlockDevice **ret_devices, size_t *ret_n_devices) {
        ...
        FOREACH_DEVICE(e, dev) {
                ...
                if (FLAGS_SET(flags, BLOCKDEV_LIST_IGNORE_ZRAM)) {
                        r = device_sysname_startswith_compat(dev, "zram");
                        if (r < 0) {
                ...
                if (FLAGS_SET(flags, BLOCKDEV_LIST_REQUIRE_PARTITION_SCANNING)) {
                        r = blockdev_partscan_enabled_compat(dev);
                        if (r < 0) {
                ...
                if (FLAGS_SET(flags, BLOCKDEV_LIST_IGNORE_EMPTY) || ret_devices) {
                        r = device_get_sysattr_u64_compat(dev, "size", &size);
                        if (r < 0)
