...
static int dns_packet_extract_question(DnsPacket *p, DnsQuestion **ret_question) {
         ...
         * contain: p->size - p->rindex are the remaining unread bytes, and 5U is the minimum size of
         * each question - 1 (QNAME) + 2 (QTYPE) + 2 (QCLASS). */
...
static int dns_packet_extract_answer(DnsPacket *p, DnsAnswer **ret_answer) {
         ...
         * p->size - p->rindex are the remaining unread bytes, and 11U is the minimum size of each RR -
         * 1 (NAME) + 2 (TYPE) + 2 (CLASS) + 4 (TTL) + 2 (RDLENGTH). */
        prealloc = (p->size - p->rindex) / 11U;
        if (prealloc == 0)
                return -EMSGSIZE;

        /* Pre-allocate the answer, but cap the pre-allocation to a number of RRs the packet can
