...
static int open_storage_dir(void) {
        ...
        r = storage_state_directory(arg_runtime_scope, &state_dir);
        if (r < 0)
                return log_error_errno(r, "Failed to get state directory path: %m");

        _cleanup_close_ int state_fd = chase_and_open(state_dir, /* root= */ NULL, CHASE_MKDIR_0755, O_CLOEXEC|O_CREAT|O_DIRECTORY, /* ret_path= */ NULL);
        if (state_fd < 0)
        ...
        _cleanup_close_ int storage_fd = chase_and_openat(state_fd, "storage", 0, O_CLOEXEC|O_DIRECTORY, /* ret_path= */ NULL);
        if (storage_fd == -ENOENT) {
                storage_fd = xopenat(state_fd, "storage", O_EXCL|O_CREAT|O_CLOEXEC|O_DIRECTORY|O_NOFOLLOW, XO_LABEL|XO_SUBVOLUME, 0700);
                if (storage_fd == -EEXIST)
                        storage_fd = chase_and_openat(state_fd, "storage", 0, O_CLOEXEC|O_DIRECTORY, /* ret_path= */ NULL);
        }
        if (storage_fd < 0)
...
static int vl_method_list_volumes(
        ...
        _cleanup_(json_variant_unrefp) JsonVariant *pending = NULL;
        ...
        FOREACH_ARRAY(dp, dentries->entries, dentries->n_entries) {
                ...
                r = chaseat(fd, d->d_name, 0, /* ret_path= */ NULL, &pin_fd);
                if (r < 0) {
                ...
                case S_IFDIR:
                        r = storage_fd_is_read_only_fs(pin_fd);
                        if (r < 0)
                ...
                case S_IFBLK: {
                        ...
                        if (r < 0)
                                ...
                                r = storage_device_get_sysattr_u64(dev, "size", &size);
                                if (r < 0)
                ...
                r = storage_varlink_reply_append(link, &pending, reply);
                if (r < 0)
                        return r;
        }

        return storage_varlink_reply_finish(link, pending, "io.systemd.StorageProvider.NoSuchVolume");
}
...
static int vl_method_list_templates(
        ...
        return storage_varlink_reply_finish(link, pending, "io.systemd.StorageProvider.NoSuchTemplate");
}
...
static int create_volume_dir(
        ...
        _cleanup_close_ int volume_fd = xopenat(storage_fd, tf, O_CREAT|O_EXCL|O_RDONLY|O_DIRECTORY|O_CLOEXEC|O_NOFOLLOW, xopen_flags, 0700);
        if (volume_fd < 0)
                return volume_fd;

        _cleanup_close_ int root_fd = xopenat(volume_fd, "root", O_CREAT|O_EXCL|O_RDONLY|O_DIRECTORY|O_CLOEXEC|O_NOFOLLOW, xopen_flags, 0755);
        if (root_fd < 0) {
                r = root_fd;
                goto fail;
        }
...
static int vl_method_acquire(
        ...
        r = chaseat(storage_fd, filename, 0, /* ret_path= */ NULL, &pin_fd);
        if (r < 0) {
                if (r != -ENOENT)
                        return r;
        ...
        if (p.request_as == VOLUME_REG) {
                ...
                r = storage_stat_verify_block(&st);
                if (r < 0)
                        return r;
        ...
        if (real_fd < 0) {
                ...
                real_fd = xopenat(pin_fd, subdir, open_flags|O_CLOEXEC, 0, /* mode= */ MODE_INVALID);
                if (real_fd < 0 && p.read_only < 0 && !S_ISDIR(st.st_mode) && ERRNO_IS_NEG_FS_WRITE_REFUSED(real_fd))
                        real_fd = xopenat(pin_fd, subdir, O_RDONLY|O_CLOEXEC, 0, /* mode= */ MODE_INVALID);
                if (real_fd < 0)
        ...
        case VOLUME_DIR: {
                ...
                _cleanup_close_ int mount_fd = storage_open_tree_attr(real_fd, p.read_only > 0);
                if (mount_fd < 0)
                ...
                r = storage_fd_is_read_only_fs(real_fd);
                if (r < 0)
                        return r;
...
static int vl_server(void) {
        ...
        r = varlink_server_new(&varlink_server, VARLINK_SERVER_ALLOW_FD_PASSING_OUTPUT);
        if (r < 0)
...
static int help(void) {
        printf("%s [OPTIONS...]\n\n"
               "Simple file system backed storage provider.\n\n"
